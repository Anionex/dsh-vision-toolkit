...
import { createEvidenceCacheKey, EvidenceCache, evidenceRuntimeFingerprint, SessionEvidenceStore, } from "./evidence-cache.js";
...
export class ImageInputVariantAdapter extends LlmAdapter {
    ...
    runtimeHash;
    lastRuntime;
    constructor(ctx, llm, upstream, upstreamName, runtime, cache, hidden = () => false, runtimeHash = () => 'process-only-runtime') {
        super();
        this.ctx = ctx;
        this.llm = llm;
    ...
    async *stream(options) {
        ...
        const messages = await convertImagesToEvidence(this.ctx, this.runtime, this.cache, options.messages, options.signal, options.sessionId === undefined ? undefined : String(options.sessionId), this.runtimeHash());
        ...
        try {
            ...
            for (const provider of providers) {
                ...
                if (disposed)
                    return;
                try {
                    const dispose = llm.registerAdapter([variantId], new ImageInputVariantAdapter(ctx, llm, upstream, provider.name, getRuntime, evidenceCache, () => getConfig().imageInputVariants.hidden, () => getRuntime()?.evidenceFingerprint ?? evidenceRuntimeFingerprint(getConfig())));
