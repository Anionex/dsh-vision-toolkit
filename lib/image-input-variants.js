...
export async function convertImagesToEvidence(ctx, runtime, cache, messages, signal) {
    ...
    const converted = await Promise.all(plans.map(async ({ message, query }) => {
        if (query === undefined)
            return message;
        const content = await convertBlocks(message.content, (block) => abortableWait(cache.read(cacheKey(String(block.attachment.attachmentId), query), () => limit(() => readImageBlock(ctx, runtime, block, query))), signal));
        return { ...message, content };
    }));
