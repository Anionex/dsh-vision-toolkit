...
async function readImageBlock(ctx, runtime, block, query, sessionId, storageDir) {
    ...
    try {
        ...
        const result = await current.glance({ images: [materialized.file], query }, {
 ...
 */
function presentUnderUpstreamRoute(upstream, messages, transcript) {
    ...
    messages.forEach((message, index) => {
        ...
        if (message.role !== 'assistant' || source.kind !== 'model')
            return;
        if (source.provider !== upstream && source.provider !== variant)
            return;
        let replayState = source.replayState;
        if (replayState === undefined) {
            durable ??= new Map((transcript() ?? [])
        ...
        if (source.provider === upstream && replayState === undefined)
            return;
        const replayed = {
            kind: 'model',
            provider: upstream,
...
function sameBlockShape(left, right) {
...
export class ImageInputVariantAdapter extends LlmAdapter {
     ...
     */
    durableTranscript(sessionId) {
        if (sessionId === undefined)
            return undefined;
        return this.ctx.sessions.get(sessionId)?.deriveMessages();
    }
    ...
    async *stream(options) {
        ...
        const delegated = presentUnderUpstreamRoute(this.upstream, messages, () => this.durableTranscript(options.sessionId));
