...
export async function labelTakeoverVerdict(ctx, label) {
    ...
    for (const provider of llm.listProviders()) {
        ...
        try {
            ...
            ctx.logger.warn('dsh-vision-toolkit: paste verdict could not read route "%s"; native paste wins for this label. %s', provider.id, messageOf(error));
            return false;
        }
 ...
 */
export function createPasteTakeoverResolver(ctx) {
