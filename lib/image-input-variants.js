...
function cacheKey(attachmentId, prompt) {
...
async function convertBlocks(blocks, convert, noteState) {
    const out = [];
 ...
 */
async function readImageBlock(ctx, runtime, block, query) {
    ...
    try {
        ...
        const result = await current.glance({ images: [file], query }, { signal: new AbortController().signal, workspace: directory });
...
export async function convertImagesToEvidence(ctx, runtime, cache, messages, signal) {
    ...
    for (const message of messages) {
        ...
        const content = await convertBlocks(message.content, (block) => abortableWait(cache.read(cacheKey(String(block.attachment.attachmentId), query), () => readImageBlock(ctx, runtime, block, query)), signal), noteState);
