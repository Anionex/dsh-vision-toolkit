...
export class ImageInputVariantAdapter extends LlmAdapter {
    ...
    hidden;
    storageDir;
    constructor(ctx, llm, upstream, upstreamName, runtime, cache, hidden = () => false, storageDir = () => undefined) {
        super();
        this.ctx = ctx;
        this.llm = llm;
    ...
    async *stream(options) {
        ...
        const messages = await convertImagesToEvidence(this.ctx, () => captured, this.cache, options.messages, options.signal, options.sessionId === undefined ? undefined : String(options.sessionId), captured?.evidenceFingerprint ?? 'process-only-runtime', this.storageDir());
        ...
        try {
            ...
            for (const provider of providers) {
                ...
                if (disposed)
                    return;
                try {
                    const dispose = llm.registerAdapter([variantId], new ImageInputVariantAdapter(ctx, llm, upstream, provider.name, getRuntime, evidenceCache, () => getConfig().imageInputVariants.hidden, () => getConfig().storageDir));
