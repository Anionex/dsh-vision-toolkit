...
function callOptions(exec, timeoutMs) {
...
const requiredHealthCheckSchema = { ...healthCheckSchema, required: true };
...
const requiredUpstreamInfoSchema = { ...upstreamInfoSchema, required: true };
function runtimeFrom(source) {
    return typeof source === 'function' ? source() : source;
}
/** Build the complete P0/P1 tool set from one live runtime source. */
export function createVisionTools(source, projectPresentation = presentationIdentity) {
    ...
    return [
        ...
        defineTool({
            ...
            execute: (args, exec) => runtimeFrom(source).health(args.testConnection === true, callOptions(exec, args.timeoutMs)),
        }),
        defineTool({
