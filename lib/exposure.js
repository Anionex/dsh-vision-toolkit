...
const PTC_DISPATCH_EVENT_TYPES = ['tool/ptc-dispatch', 'tool/code-dispatch'];
/**
 ...
 */
function isBundledSkillPtcDispatch(event) {
    const type = event.type;
    if (!PTC_DISPATCH_EVENT_TYPES.includes(type))
        return false;
...
function hasLoadedVisionSkill(session) {
    ...
    for (const event of events) {
        ...
        if (isBundledSkillPtcDispatch(event))
            return true;
    }
    return false;
}
