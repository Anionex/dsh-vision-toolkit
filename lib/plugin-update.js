            ...
            if (updateAttempted && updateContext !== undefined && updateBackup !== undefined) {
                ...
                try {
                    ...
                    if (rollback.exitCode !== 0) {
                        rollbackFailure = new PluginUpdateError('update-rollback-failed', publicCommandFailure(rollback, 'Plugin update failed and the previous version could not be restored'), { cause: error });
                    }
                    else {
                        ...
                        if (restored.version !== this.currentVersion) {
                            rollbackFailure = new PluginUpdateError('update-rollback-failed', `Plugin update failed and rollback installed ${String(restored.version)} instead of ${this.currentVersion}`, { cause: error });
                        }
                    }
                }
                catch (rollbackError) {
                ...
                try {
                    ...
                    rollbackFailure ??= new PluginUpdateError('update-rollback-failed', 'Plugin update failed and its recovery backup could not be removed', { cause: cleanupError });
                }
