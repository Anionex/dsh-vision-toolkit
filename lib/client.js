...
const en = {
    ...
    storageDirHint: 'Leave blank to keep .dsh-vision-toolkit in each workspace. Set an absolute shared root such as /tmp/dsh-vision-toolkit to store artifacts, pasted images, and caches in an automatically generated per-user, workspace-specific child directory.',
...
const zh = {
    ...
    storage: '本地文件',
    storageDir: '默认保存目录',
    storageDirHint: '留空时继续在各工作区创建 .dsh-vision-toolkit。填写绝对路径共享根目录（例如 /tmp/dsh-vision-toolkit）后，产物、粘贴图片和缓存会保存到自动生成的当前用户、工作区专属子目录中。',
