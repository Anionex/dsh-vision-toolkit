...
class PasteImageController {
    ...
    verdictFor(sessionId) {
        ...
        if (entry === undefined || entry.at === 0 || !entry.takeOver)
            return false;
    ...
    refreshVerdict(sessionId) {
        ...
        let request;
        try {
            request = fetch(`${exports.PASTE_POLICY_ROUTE}?sessionId=${encodeURIComponent(sessionId)}`);
        }
        catch {
...
function installPasteImages(ctx) {
    ...
    ctx.effect(() => {
        ...
        const onFocusIn = () => {
            ...
            if (sessionId !== undefined)
                controller.refreshVerdict(String(sessionId));
        };
