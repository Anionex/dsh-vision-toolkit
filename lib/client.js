...
function HealthToolView({ block, t = key => en[key] }) {
    const value = decodeVisionResult(block);
    const checks = isRecord(value?.checks) ? Object.entries(value.checks).filter((entry) => isRecord(entry[1])) : [];
    const healthy = value?.healthy === true;
    return ((0, jsx_runtime_1.jsx)(ToolShell, { block: block, title: "Vision Health", summary: healthy ? t('healthy') : t('degraded'), icon: (0, jsx_runtime_1.jsx)(VisionIcon, {}), t: t, children: checks.length === 0 ? (0, jsx_runtime_1.jsx)("p", { className: "dvt-muted", children: t('noResult') }) : (0, jsx_runtime_1.jsx)("div", { className: "dvt-health-grid", children: checks.map(([name, check]) => (0, jsx_runtime_1.jsxs)("div", { "data-status": stringOf(check.status), children: [(0, jsx_runtime_1.jsx)("span", { children: name }), (0, jsx_runtime_1.jsx)("strong", { children: stringOf(check.status) ?? '—' }), (0, jsx_runtime_1.jsx)("p", { children: stringOf(check.detail) ?? '' })] }, name)) }) }));
}
