 ...
 */
function draftAttachmentCount(snapshot) {
    if (snapshot === null || typeof snapshot !== 'object')
        return 0;
    const row = snapshot;
    if (Array.isArray(row.attachmentIds))
        return row.attachmentIds.length;
...
class PasteImageController {
    ...
    inputFor(sessionId) {
        const actx = sessionRegistry(this.ctx).scope(sessionId);
        if (actx === undefined)
