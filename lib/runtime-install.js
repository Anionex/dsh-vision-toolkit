    ...
    await withDirectoryLock(`${root}.lock`, async () => {
        ...
        try {
            ...
            await withWindowsTransientRetry(() => rm(work, { recursive: true, force: true })).catch(error => {
                ctx.logger.warn('dsh-vision-toolkit: bundled Python staging cleanup failed: %s', error instanceof Error ? error.message : String(error));
            });
        }
    });
...
async function prepareManaged(ctx, config, manifest) {
    ...
    try {
        ...
        await withWindowsTransientRetry(() => rm(quarantine, { recursive: true, force: true })).catch(error => {
            ctx.logger.warn('dsh-vision-toolkit: managed runtime quarantine cleanup failed: %s', error instanceof Error ? error.message : String(error));
        });
        const python = { program: interpreter, prefix: [], display: interpreter };
        return { source: 'managed', root: BUNDLED_ROOT, python, cleanHome, pythonVersion: metadata.version, dependencies };
    }
        ...
        await withWindowsTransientRetry(() => rm(staging, { recursive: true, force: true })).catch(error => {
            ctx.logger.warn('dsh-vision-toolkit: managed runtime staging cleanup failed: %s', error instanceof Error ? error.message : String(error));
        });
