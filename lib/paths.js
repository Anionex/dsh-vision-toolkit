...
function assertSecureSharedBase(info, requested) {
...
export async function resolveWorkspaceStorage(workspaceRaw, storageDirRaw) {
    ...
    try {
        ...
        if (info.isSymbolicLink())
            throw new Error('configured storage path must not be a symbolic link');
        if (!info.isDirectory())
            throw new Error('configured storage path is not a directory');
