...
export function createVisionProviderHeaderResolver(hmacKey = randomBytes(32)) {
    if (hmacKey.byteLength < 32)
        throw new TypeError('vision provider session HMAC key must contain at least 32 bytes');
    const key = Buffer.from(hmacKey);
    return (provider, operationKey) => {
        if (provider.sessionHeaders.length === 0)
...
export class VisionToolkitRuntime {
    ...
    async captureEvidenceRuntime() {
        ...
        return Object.freeze({
            evidenceFingerprint,
            glance: (request, options) => this.glanceWithEnv(request, options, env),
        });
    }
    timeout(options) {
