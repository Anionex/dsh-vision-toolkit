...
import { sessionHeadersForOperation } from "./session-headers.js";
...
function sessionHeaderSecrets(serialized) {
...
export class VisionToolkitRuntime {
    ...
    async captureEvidenceRuntime() {
        ...
        const evidenceFingerprint = evidenceRuntimeFingerprint(this.config, createHash('sha256').update(env.VISION_API_KEY).digest('hex'), env.VISION_SSL_VERIFY);
        return Object.freeze({
            evidenceFingerprint,
            glance: (request, options) => this.glanceWithEnv(request, options, this.visionEnv(resolved, options)),
        });
    }
    timeout(options) {
    ...
    async resolveVisionEnv(options) {
    ...
    async glanceWithEnv(request, options, capturedEnv) {
        ...
        return this.runOperation('vision_toolkit_health', options, async (operation) => {
            ...
            if (testModel) {
                if (resolvedCredential === undefined) {
                    ...
                    try {
                        const result = await this.runUpstream('glance', [VISION_MODEL_TEST_IMAGE, '-q', VISION_MODEL_TEST_PROMPT], operation, this.visionEnv(resolvedCredential, options));
