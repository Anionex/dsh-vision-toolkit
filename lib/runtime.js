...
export class VisionToolkitRuntime {
    ...
    async readCacheCandidate(root, name, expectedOutDigest, maxBytes, maxPixels, operation) {
        ...
        if (bytes.length !== info.size || createHash('sha256').update(bytes).digest('hex') !== expectedOutDigest)
            return undefined;
        let probed;
        try {
    ...
    async autoCompressImage(image, policy, operation) {
        ...
        for (const entry of await readdir(root)) {
            ...
            const cached = await this.readCacheCandidate(root, entry, outDigest, this.config.maxImageBytes, this.config.maxImagePixels, operation);
            if (cached !== undefined) {
