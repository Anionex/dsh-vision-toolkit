...
export async function apply(ctx, config = {}) {
    ...
    const ensureOperational = () => {
        ...
        let skill;
        try {
            for (const tool of createVisionTools(() => manager.current(), value => artifacts.presentationMeta(value), lifecycle.signal)) {
            ...
            if (skill !== undefined)
                skill();
            for (const dispose of tools.reverse())
                dispose();
            throw error;
        }
    };
    ...
    return () => {
        ...
        if (operationalDisposers !== undefined) {
            for (const dispose of operationalDisposers.tools.reverse())
                dispose();
