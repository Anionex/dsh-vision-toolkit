![DSH Vision Toolkit running real vision_trace and vision_pixel_diff conversations in the DeepSeek Harness Web UI](assets/dsh-conversation-pixel-diff.png)
...
## Real DSH conversations

These screenshots are live DeepSeek Harness Web sessions, not upstream reference runs. They show `/vision-tools` loading, an intent-aware image Q&A turn, a real `vision_trace` call with its SVG Artifact card, a real `vision_pixel_diff` run, and the screenshot-debugging conversation that compares the checked-in UI-restoration fixture.

<p align="center">
  <img src="assets/dsh-conversation-tool-call.png" width="49%" alt="DSH Web session with the /vision-tools command and vision_toolkit_activate visible." />
  <img src="assets/dsh-conversation-artifact.png" width="49%" alt="DSH Web Artifact card for sample-trace.svg generated by vision_trace." />
</p>

<p align="center">
  <img src="assets/dsh-conversation-vision-trace.png" width="49%" alt="DSH Web final result for the vision_trace conversation." />
  <img src="assets/dsh-conversation-pixel-diff.png" width="49%" alt="DSH Web final result for the vision_pixel_diff conversation." />
</p>

<p align="center">
  <img src="assets/dsh-conversation-image-qa-top.png" width="49%" alt="DSH Web session showing the user asking about an image while /vision-tools activates." />
  <img src="assets/dsh-conversation-image-qa.png" width="49%" alt="DSH Web final reply from an image Q&A conversation using vision_glance and local color tools." />
</p>

<p align="center">
  <img src="assets/dsh-conversation-screenshot-debugging-top.png" width="49%" alt="DSH Web session starting screenshot debugging with vision_glance over the initial and reference UI images." />
  <img src="assets/dsh-conversation-screenshot-debugging.png" width="49%" alt="DSH Web final reply listing the concrete UI differences before continuing to vision_pixel_diff." />
</p>

## DSH-native proof: reference-to-pixel verification

The checked-in UI-restoration workflow renders an intentionally inaccurate HTML implementation, measures a `6.04%` pixel difference across six non-zero regions, iterates, and reaches an exact `0%` difference against the reference at `1200 × 720`. The fourth screenshot above is the same `vision_pixel_diff` comparison run inside the DSH Web UI; the screenshot-debugging conversation above first uses `vision_glance` over the same initial and reference images.
