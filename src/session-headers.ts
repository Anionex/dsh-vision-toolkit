...
export function normalizeSessionHeaderNames(input: readonly string[]): string[] {
  ...
  for (const raw of input) {
    ...
    if (name.length === 0) throw new TypeError('provider.sessionHeaders must not contain an empty name')
...
export function createSessionHeaderRouter(secret: Uint8Array = randomBytes(32)): SessionHeaderRouter {
...
export function sessionHeadersForOperation(names: readonly string[], operationKey: string): Record<string, string> {
