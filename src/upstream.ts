...
const COMPRESS_IMAGE_SCRIPT = [
  ...
  'def save_candidate(im,path,fmt,**kwargs):',
  ...
  'def lossless_savers(im,fmt):',
  '    savers=[]',
  '    if fmt=="png":',
  '        savers.append(("png-optimized","png",False,lambda:save_candidate(im,dest,"PNG",optimize=True)))',
  '    elif fmt in ("jpeg","jpg"):',
  '        savers.append(("jpeg-q95","jpeg",False,lambda:save_candidate(im,dest,"JPEG",quality=95,optimize=True,progressive=True)))',
  '    elif fmt=="webp":',
  '        savers.append(("webp-lossless","webp",False,lambda:save_candidate(im,dest,"WEBP",lossless=True,quality=100,method=6)))',
  '    elif fmt=="gif":',
  '        savers.append(("gif-optimized","gif",False,lambda:save_candidate(im,dest,"GIF",optimize=True)))',
  '    if fmt not in ("png","gif"):',
  '        savers.append(("png-optimized","png",False,lambda:save_candidate(im,dest,"PNG",optimize=True)))',
  '    if fmt!="webp":',
  '        savers.append(("webp-lossless","webp",False,lambda:save_candidate(im,dest,"WEBP",lossless=True,quality=100,method=6)))',
  '    return savers',
  'def lossy_savers(im,alpha):',
  '    savers=[]',
  '    if alpha:',
  '        for q in qualities:',
  '            savers.append(("webp-q%d"%q,"webp",True,lambda q=q:save_candidate(im,dest,"WEBP",quality=q,method=6)))',
  '        savers.append(("png-palette-256","png",True,lambda:save_candidate(im.quantize(colors=256),dest,"PNG",optimize=True)))',
  '        for q in qualities:',
  '            savers.append(("jpeg-q%d-flatten"%q,"jpeg",True,lambda q=q:save_candidate(flatten(im),dest,"JPEG",quality=q,optimize=True,progressive=True)))',
  '    else:',
  '        for q in qualities:',
  '            savers.append(("jpeg-q%d"%q,"jpeg",True,lambda q=q:save_candidate(im,dest,"JPEG",quality=q,optimize=True,progressive=True)))',
  '        for q in qualities:',
  '            savers.append(("webp-q%d"%q,"webp",True,lambda q=q:save_candidate(im,dest,"WEBP",quality=q,method=6)))',
  '    return savers',
  'try:',
  ...
  '    if w*h<=max_pixels:',
  '        candidates=lossless_savers(current,source_format)',
  '        candidates.extend(lossy_savers(current,has_alpha(current)))',
  '    else:',
  '        candidates=[]',
  ...
  '            continue',
  '        if ok(w,h,size):',
  '            print(json.dumps({"ok":True,"bytes":size,"width":w,"height":h,"format":fmt,"mode":mode,"lossy":lossy,"resized":step>0,"candidate":label}))',
  '            sys.exit(0)',
...
export class UpstreamAdapter {
  ...
  async compressImage(
    ...
    const { bytes, width, height, format, mode, lossy, resized, candidate } = record
    if (
