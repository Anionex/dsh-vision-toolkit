...
export interface TraceReport {
  ...
  perception?: { label?: string; confidence?: unknown }
  geometry: {
    status: string
    confidence: unknown
...
export function parseLocationOutput(stdout: string): LocatedElement[] {
  ...
  for (const line of stdout.split(/\r?\n/)) {
    if (line.trim().length === 0 || line.trim() === 'no elements detected') continue
    const parsed = parseLocationLine(line)
    if (parsed !== undefined) elements.push(parsed)
  }
  return elements
}
...
export function parseTraceReport(text: string): TraceReport {
  let payload: unknown
  try {
  ...
  const size: [number, number] = Array.isArray(logicalSize) && logicalSize.length >= 2
  ...
  return {
    ...
    ...(typeof perception === 'object' && perception !== null
      ? {
        perception: {
          ...(typeof (perception as Record<string, unknown>).label === 'string'
            ? { label: (perception as Record<string, unknown>).label as string }
            : {}),
          ...((perception as Record<string, unknown>).confidence !== undefined
            ? { confidence: (perception as Record<string, unknown>).confidence }
            : {}),
        },
      }
      : {}),
...
export function defaultUpstreamCandidates(): string[] {
...
const TOOL_SCRIPTS = ['glance', 'ground', 'detect', 'crop', 'trace'] as const
...
export async function findCheckout(candidates: readonly string[]): Promise<string> {
  ...
  for (const candidate of candidates) {
    ...
    if (await isCheckout(real)) return real
    attempts.push(`${candidate} (missing required bin scripts)`)
  }
  throw new VisionToolkitError(
    'runtime',
    `agent-vision-toolkit checkout not found; tried: ${attempts.join('; ')}; clone https://github.com/Anionex/agent-vision-toolkit at commit ${UPSTREAM_COMMIT} and configure runtime.agentVisionToolkitPath`,
  )
}
...
export class UpstreamAdapter {
  private version: UpstreamVersionInfo
  ...
  async run(
    ...
    if (this.version.path.length === 0) await this.prepare()
    ...
    try {
      ...
      throw new VisionToolkitError(
        'runtime',
        `${tool}: failed to start ${this.config.runtime.python}`,
        { cause: error },
      )
    }
  }
  ...
  async probeImageSize(
    ...
    if (outcome.outcome.exitCode !== 0) {
      ...
      throw new VisionToolkitError(
        'output',
        `cannot read image dimensions: ${outcome.stderr.trim() || 'unknown error'}`,
      )
    }
  ...
  private async collectProbe(handle: SubprocessHandle): Promise<UpstreamRunResult> {
    const outcome = await handle.done
    const stdout = handle.collected.stdout?.readFrom(0)
    const stderr = handle.collected.stderr?.readFrom(0)
    return {
   ...
   */
  async readCheckoutVersion(): Promise<string> {
    if (this.version.path.length === 0) await this.prepare()
    try {
  ...
  classifyFailure(
    ...
    if (options.timedOut) return new VisionToolkitError('cancelled', `${tool}: timed out`)
