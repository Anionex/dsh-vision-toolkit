...
export async function savePastedImages(
...
): Promise<{ content: PromptContentPart[]; saved: SavedPastedImage[] }> {
  ...
  if (saved.length > 0) blocks.push({ type: 'text', text: pastedImageNote(saved) })
  return { content: blocks, saved }
}

/** Model-visible note naming the saved pasted images and the tool to read them. */
export function pastedImageNote(saved: readonly SavedPastedImage[]): string {
 ...
 */
export function stripImageBlocks(messages: readonly Message[]): Message[] | undefined {
  let changed = false
