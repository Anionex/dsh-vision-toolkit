![DSH Vision Toolkit 在 DeepSeek Harness Web UI 中运行真实的 vision_trace 与 vision_pixel_diff 对话](assets/dsh-conversation-pixel-diff.png)
...
以下截图均为真实 DeepSeek Harness Web 会话，不是上游参考图。它们展示了 `/vision-tools` 加载、一次带意图的图片问答、一次真实的 `vision_trace` 调用及其 SVG 产物卡片、一次真实的 `vision_pixel_diff` 运行，以及针对仓库 UI 还原夹具进行截图排障的对话。

<p align="center">
  <img src="assets/dsh-conversation-tool-call.png" width="49%" alt="显示 /vision-tools 命令和 vision_toolkit_activate 的 DSH Web 会话。" />
  <img src="assets/dsh-conversation-artifact.png" width="49%" alt="DSH Web 中由 vision_trace 生成的 sample-trace.svg 产物卡片。" />
</p>

<p align="center">
  <img src="assets/dsh-conversation-vision-trace.png" width="49%" alt="vision_trace 对话的 DSH Web 最终结果。" />
  <img src="assets/dsh-conversation-pixel-diff.png" width="49%" alt="vision_pixel_diff 对话的 DSH Web 最终结果。" />
</p>

<p align="center">
  <img src="assets/dsh-conversation-image-qa-top.png" width="49%" alt="DSH Web 会话中用户向图片提问并加载 /vision-tools 的界面。" />
  <img src="assets/dsh-conversation-image-qa.png" width="49%" alt="DSH Web 中图片问答的最终回复，使用了 vision_glance 和本地颜色工具。" />
</p>

<p align="center">
  <img src="assets/dsh-conversation-screenshot-debugging-top.png" width="49%" alt="DSH Web 会话开始用 vision_glance 对比 initial.png 和 reference.png 进行截图排障。" />
  <img src="assets/dsh-conversation-screenshot-debugging.png" width="49%" alt="DSH Web 最终回复列出的具体 UI 差异，并建议继续运行 vision_pixel_diff。" />
</p>

## DSH 原生实证：从参考图到像素级一致

仓库中的 UI 还原流程会渲染一个故意不准确的 HTML 实现，测得 `6.04%` 像素差异和 6 个非零差异区域；经过迭代后，在 `1200 × 720` 下达到相对参考图精确 `0%` 的差异。上面的第四张截图就是同一次 `vision_pixel_diff` 比较在 DSH Web UI 中的实况；后面的截图排障对话先用 `vision_glance` 对比同一对 initial 与 reference 图片。
